...
作者（coderlang）从 web2,web3 的角度，横向，纵向，分享了公私钥加解密（ECC）及签名的应用，你也可以下载 Dsyncle 体验。

在 iOS, Android, 网页端落地过程中，也遇到一些困难，找不到一个统一的 ECC 加密三方库。

有一晚，突然意识到，可以用 rust 、golang 编译为 iOS,Android,网页调用的库，这样统一了各端算法，最终，使用了 rust，虽然，团队没有一个人会，但不影响，我们使用。