...
你在授权接入节点时，存入节点的数据，都用你的私钥进行了签名，公钥进行了加解密，你在提取数据时，也需要你的签名，节点签名验证过了，才会返回数据给你，而你拿到数据，则可以验证节点是否修改过你留下的数据。
...
在 web3 里，私钥就是你的一切，忘记私钥，你就找不回你的数据，资产。

私钥通常由128位二进制数据组成，为了记住私钥，行业提出了众多解决方案，通过 BIP39 协议，将 128位二进制数据，转换为 12 个单词，即常见的「助记词」，例如：```voice true main glow spare magic you snack dress summer cherry cash```就是一套助记词。

通过私钥可以推导出公钥，公钥和私钥是成对的，它们有 2 个作用：**加密解密**和**签名验签**，在 web3 的产品里，都会应用到。
...
虽然，第 8 步，以后，Server1 访问 Signing Machine，不再需要 UnionAuth，但当 Token 过期了，又需要借助 UnionAuth 从第 1 步开始执行，获取 Token。

这种方案，有一个问题，就是单机故障，只要 UnionAuth 服务挂了，整套系统，都会受到严重的影响。


### Web3 服务间调用的实现

接下来，一起思考下，在 Web3 上，如何解决 UnionAuth 单机故障？

在 Web3上，不是解决 UnionAuth 单机故障，而是不再需要 UnionAuth，则依然可以，实现服务之间的安全访问，访问过程，缩短到只需要 3 步，如下图所示：
...
Token 过期了，再次执行第一步，则可以实现，自动登录，时序图如下：

![NoUnionAuthTiming.jpg](NoUnionAuthTiming.jpg)

请再思考一个问题，第 1,2 步请求登录的签名数据，可能导致重放，即用过了，还可以使用，如何解决？

为了，解决这个问题，可以增加 Nonce，每次加 1，Signing Machine 维护，用过即废。

## 四、应用（二）：账号登录

请你回忆一下，在 Web2 的场景里，你是怎么做账号登录的，常见的方式有手机号 + 验证码，邮箱 + 验证码。

登录时，服务器验证码验证过了，通过数据库的唯一索引，给你分配一个唯一的 UID。

Web3 不叫登录，而是授权，通过你的授权，签名数据，节点（服务器）可以推导出你公钥对应的唯一地址，如：0xc3eA7f07Aa485762a5687074c91D9aA9E5Af1d29，而地址就是 web2 的 UID 。
